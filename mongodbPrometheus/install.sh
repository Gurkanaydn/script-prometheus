...
sudo wget https://github.com/percona/mongodb_exporter/releases/download/v0.31.1/mongodb_exporter-0.31.1.linux-amd64.tar.gz
sudo tar xvf mongodb_exporter-0.31.1.linux-amd64.tar.gz
cd mongodb_exporter-0.31.1.linux-amd64
sudo useradd -rs /bin/false prometheus
sudo cp mongodb_exporter /usr/local/bin 
export MONGODB_URI=mongodb://mongodb_exporter:redhat@0.0.0.0:27017
sudo systemctl restart mongod
cd ..
sudo cp mongodb_exporter.service /lib/systemd/system/
sudo systemctl daemon-reload
